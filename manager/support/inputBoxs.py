...
def drawInputBoxs(screen,inputBoxs,input_limit,events,mouse_pos,y_start, x_start, boxDim, space_between_box):
    for key,boxInfo in inputBoxs.items():
        ...
        if boxInfo[0] != '':
            text_surface = pygame.font.SysFont("arial", 12).render(key, True, (255, 255, 255))
        else:
            text_surface = pygame.font.SysFont("arial", 12).render(key, True, Color.red1.value)
        ...
        text_surface = pygame.font.SysFont("arial", 20).render(inputBoxs[key][0], True, (255, 255, 255))
        size = pygame.font.Font.size(pygame.font.SysFont("arial", 20), inputBoxs[key][0])

        # bliting the text in the center of the box
        screen.blit(text_surface, (x_start+boxDim[0]/2-size[0]/2,y_start+9))
